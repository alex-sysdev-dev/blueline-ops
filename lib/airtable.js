...
/**
 * Fetches the Facilities network data from Airtable.
 */
export async function getFacilities() {
  // Now using your BASE_ID variable!
  const url = `https://api.airtable.com/v0/${BASE_ID}/Facilities`;
  ...
  try {
    ...
    return data.records.map((record) => ({
      id: record.id,
      name: record.fields.Name || 'Unknown Facility',
      status: record.fields.Status || 'Optimal',
      headcount: record.fields.Headcount || 0,
      uplh: record.fields.UPLH || 0,
      issues: record.fields.Issues || 0
    }));

  } catch (error) {
    console.error("Airtable Facilities error:", error);
    return []; 
  }
}